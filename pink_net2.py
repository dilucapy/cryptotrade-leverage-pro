...
def plot_open_orders(data_asset, current_price, active_symbol):
    ...
    if non_mother_profits:
        ...
        # Añadir etiquetas sobre cada barra con profits, porcentajes y precios
        for bar, profit, percentage, price, amount_usdt in zip(bars2, non_mother_profits, non_mother_percentages, non_mother_prices,
                                                               non_mother_amounts_usdt):
            yval = bar.get_height()
        ...
        ax2.text(0.5, 0.5, "No se encontraron datos", ha='center', va='center', fontsize=14, color='blue', weight='bold')
...
def select_asset(data):
    """Solicita al usuario que seleccione un activo y devuelve los datos del activo."""
    symbol = input("Introduce el símbolo del activo (ejemplo: BTC): ").strip().upper()
...
def remove_buy_limit(data, data_asset, active_symbol, filename ):
    ...
    # Verificar si se encontró una orden antes de intentar eliminarla
    if order is not None:
        confirmation = input(f"¿Está seguro de que desea eliminar la BUY LIMIT a {order['price']}? (s/n): ").strip().lower()
        if confirmation == 's':
            try:
...
def remove_sell_limit(data, data_asset, active_symbol, filename ):
    ...
    # Verificar si se encontró una orden antes de intentar eliminarla
    if order is not None:
        confirmation = input(f"¿Está seguro de que desea eliminar la SELL LIMIT a {order['price']}? (s/n): ").strip().lower()
        if confirmation == 's':
            try:
...
def update_and_save_mother_order(data, data_asset, amount_usdt_mother_order, price_mother_order, quantity_mother_order, active_symbol,
                                 filename):
        ...
        if active_symbol == '1':
            ...
            while True:
                ...
                if opcion == '1':
                    ...
                    save_data_asset(data, data_asset, active_symbol, filename)  # Guardar cambios después de cada adición"""
                    ...
                    clear_console()
                    amount_usdt_mother_order, price_mother_order, quantity_mother_order = calculate_mother_order(active_symbol)
                    update_and_save_mother_order(data, data_asset, amount_usdt_mother_order, price_mother_order, quantity_mother_order, active_symbol, filename)
